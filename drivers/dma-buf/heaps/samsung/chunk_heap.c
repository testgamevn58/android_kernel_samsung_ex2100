...
static int chunk_heap_probe(struct platform_device *pdev)
{
	...
	if (!IS_ENABLED(CONFIG_EXPERIMENTAL_DMA_BUF)) {
		pr_info("CONFIG_EXPERIMENTAL_DMA_BUF is disabled so chunk dma-heap can't probe\n");
		return -ENODEV;
	}
...
{
	if (!IS_ENABLED(CONFIG_EXPERIMENTAL_DMA_BUF)) {
		pr_info("CONFIG_EXPERIMENTAL_DMA_BUF is disabled so chunk dma-heap driver can't probe\n");
		return 0;
	}
